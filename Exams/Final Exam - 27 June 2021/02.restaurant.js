class Restaurant {
    ...
    addToMenu(meal, neededProducts, price){
        ...
        this.menu[meal] = {neededProducts:[...neededProducts], price: Number(price)}
    ...
    showTheMenu(){
      ...
      return  Object.keys(this.menu).length === 0 ? `Our menu is not ready yet, please come later...`
        : Object.keys(this.menu).map(product => `${product} - $ ${this.menu[product].price}`).join('\n')
    }
    ...
    makeTheOrder(meal) {
        ...
        currentMeal.neededProducts.forEach((p) => {
